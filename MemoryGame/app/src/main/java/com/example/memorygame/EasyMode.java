...
public class EasyMode extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState)  {
        ...
        buttonList = new ArrayList<Button>(Arrays.asList(findViewById(R.id.button6), findViewById(R.id.button5), findViewById(R.id.button4),
                findViewById(R.id.button2), findViewById(R.id.button3), findViewById(R.id.button), findViewById(R.id.button7), findViewById(R.id.button8),
                findViewById(R.id.button9), findViewById(R.id.button10), findViewById(R.id.button14), findViewById(R.id.button15)));
    ...
    private void onClick(View view, ArrayList<Integer> result){
        ...
        moves++;
        moveTV= (TextView) findViewById(R.id.move);
        moveTV.setText("Moves: " + moves);

        if(checkAllFaceUp()){
            bestMoveTV = (TextView) findViewById(R.id.bestmoveE);
            SharedPreferences prefs = this.getSharedPreferences("myPrefsKey", Context.MODE_PRIVATE);
            bestMove = prefs.getInt("key", 0);
            if(bestMove == 0 || moves <= bestMove ){
                bestMove = moves;
                SharedPreferences.Editor edit = prefs.edit();
                edit.putInt("key", moves);
                edit.commit();
            }
            ...
            final Runnable runnable = new Runnable() {
                @Override
                public void run() {
                    Intent intent = new Intent(EasyMode.this, Main.class);
                    startActivity(intent);
                }
            };
